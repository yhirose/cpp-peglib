...
// 	just a consumer called before and after each parse()
class WsRule;
...
public:
	...
	virtual ~Ope() {};
	virtual size_t parse(const char* s, size_t n, SemanticValues& sv, Context& c, any& dt, WsRule& wsr) const = 0;
	virtual void accept(Visitor& v) = 0;
};

// inherit from rule so we can use rest of mapping/grammar construction mechanism
class WsRule : public Ope
{
public:
	//
	WsRule()	{}
	// just a consumer called before and after each parse()
	virtual size_t parse(const char* s, size_t n, SemanticValues& sv, Context& c, any& dt, WsRule&) const
	{
		return n;
	}
	//
	virtual void accept(Visitor&) { /* nop */ }
};
...
public:
	...
	size_t parse(const char* s, size_t n, SemanticValues& sv, Context& c, any& dt, WsRule& wsr) const override {
		size_t i = 0;
		for (const auto& ope : opes_) {
			const auto& rule = *ope;
			auto len = rule.parse(s + i, n - i, sv, c, dt, wsr);
			if (fail(len)) {
				return -1;
			}
			i += len;
...
public:
	...
	size_t parse(const char* s, size_t n, SemanticValues& sv, Context& c, any& dt,WsRule& wsr) const override {
		...
		for (const auto& ope : opes_) {
			const auto& rule = *ope;
			auto& chldsv = c.push();
			auto len = rule.parse(s, n, chldsv, c, dt, wsr);
			if (len != -1) {
				if (!chldsv.empty()) {
...
public:
	...
	size_t parse(const char* s, size_t n, SemanticValues& sv, Context& c, any& dt, WsRule& wsr) const override {
		const auto& rule = *ope_;
		auto len = rule.parse(s, n, sv, c, dt, wsr);
		if (fail(len)) {
			return -1;
		}
		auto save_error_pos = c.error_pos;
...
public:
	...
	size_t parse(const char* s, size_t n, SemanticValues& sv, Context& c, any& dt,WsRule& wsr) const override {
		...
		assert(ope);
		const auto& rule = *ope;
		return rule.parse(s, n, sv, c, dt, wsr);
	}
...
private:
	...
	Result parse_core(const char* s, size_t n, SemanticValues& sv, any& dt, const char* path, WsRule& wsr = WsRule()) const {
		AssignIDToDefinition assignId;
		holder_->accept(assignId);

		Context cxt(path, s, n, assignId.ids.size(), enablePackratParsing);
		auto len = holder_->parse(s, n, sv, cxt, dt, wsr);
...
inline size_t Holder::parse(const char* s, size_t n, SemanticValues& sv, Context& c, any& dt, WsRule& wsr) const {
	if (!ope_) {
	...
	c.packrat(s, outer_->id, len, val, [&](any& val) {
		...
		const auto& rule = *ope_;
		len = rule.parse(s, n, chldsv, c, dt, wsr);
...
inline size_t DefinitionReference::parse(
	const char* s, size_t n, SemanticValues& sv, Context& c, any& dt, WsRule& wsr) const {
...
public:
	...
	bool parse_n(const char* s, size_t n, const char* path = nullptr) const {
		if (grammar_ != nullptr) {
			...
			// null implementation
			WsRule& wsr = WsRule();
			// check to see if grammer defined custom whitespace rule ...
			if (grammar_->find("WS") != grammar_->end())
			{
				// ...
			}
